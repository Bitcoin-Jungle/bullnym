...
pub struct DonationPage {
    ...
    pub pos_ct_descriptor: Option<String>,
    pub pos_next_addr_idx: i32,
    pub header: String,
    ...
    sqlx::query_as::<_, DonationPage>(
        "INSERT INTO donation_pages \
         ...
         ON CONFLICT (nym) DO UPDATE SET \
             ...
             archived_at = NULL, \
             updated_at = now() \
         RETURNING nym, ct_descriptor, next_addr_idx, pos_ct_descriptor, pos_next_addr_idx, \
                   header, description, avatar_sha256, og_sha256, \
...
pub async fn get_donation_page_by_nym(
    ...
    sqlx::query_as::<_, DonationPage>(
        "SELECT nym, header, description, avatar_sha256, og_sha256, \
                ct_descriptor, next_addr_idx, pos_ct_descriptor, pos_next_addr_idx, \
                display_currency, website, twitter, \
                instagram, enabled, (archived_at IS NOT NULL) AS is_archived \
...
/// Bump `donation_pages.next_addr_idx` for a non-archived page and derive the
/// next Liquid address from the page-specific descriptor.
...
/// Bump `donation_pages.pos_next_addr_idx` for a non-archived page and derive
/// the next Liquid address from the POS-specific descriptor. This never falls
/// back to the donation-page descriptor: a terminal invoice without a POS
/// descriptor is a provisioning error, not a mixed-funds checkout.
pub async fn allocate_next_liquid_for_pos_terminal<F>(
    pool: &PgPool,
    nym: &str,
    derive_address: F,
) -> Result<Option<(String, i32, String)>, sqlx::Error>
    ...
    F: Fn(&str, u32) -> Result<String, sqlx::Error>,
{
    ...
    sqlx::query("SELECT pg_advisory_xact_lock(hashtext($1)::bigint)")
        .bind(format!("donation-page-pos:{nym}"))
        .execute(&mut *tx)
        .await?;

    let row: Option<(String, i32)> = sqlx::query_as(
        "SELECT pos_ct_descriptor, pos_next_addr_idx \
         FROM donation_pages \
         WHERE nym = $1 \
           AND archived_at IS NULL \
           AND pos_ct_descriptor IS NOT NULL",
    )
    .bind(nym)
    .fetch_optional(&mut *tx)
    .await?;
    ...
    for _ in 0..100 {
        let idx_u32 = u32::try_from(address_index).map_err(|_| {
            sqlx::Error::Protocol(format!("POS address index overflow: {address_index}"))
        })?;
        let address = derive_address(&ct_descriptor, idx_u32)?;
        let in_use: bool = sqlx::query_scalar(
            "SELECT EXISTS( \
        ...
        if !in_use {
            sqlx::query("UPDATE donation_pages SET pos_next_addr_idx = $2 WHERE nym = $1")
                .bind(nym)
                .bind(address_index + 1)
                .execute(&mut *tx)
                .await?;
    ...
    tx.commit().await?;
    Err(sqlx::Error::Protocol(format!(
        "could not allocate unused POS Liquid address for {nym} after 100 attempts"
    )))
}
